  ...
  persist(
    (set) => ({
      ...
      setGraphMaxNodes: (nodes: number, triggerRefresh: boolean = false) => {
        set((state) => {
          const updates: Partial<SettingsState> = { graphMaxNodes: nodes };

          // Nếu cần refresh, tăng trigger lên 1 đơn vị
          if (triggerRefresh) {
            updates.graphRefreshTrigger = state.graphRefreshTrigger + 1;
          }
